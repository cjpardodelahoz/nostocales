...
L1648_loci <- scan(file = "misc_files/L1648.txt", what = "character") %>%
  stringr::str_remove("_selected_55") %>%
  sort()
...
dash_55_na_ng_summary <- dash_55_na_ng_summary %>%        # na df
  tibble::add_column(loci_names_198 = 
               sort(L1648_loci))
...
RootsExtremaInflections::inflexi(as.numeric(dash_55_na_ng.summary$mean_ufboot), 
                                 as.numeric(dash_55_na_ng.summary$No_variable_sites),
                                 i1 = 40, i2 = 100, nt = 2)
# Find inflexion point on bootstrap axis for amino acids
# The inflection point is indicated by the dashed red line
RootsExtremaInflections::inflexi(as.numeric(dash_55_aa_ng_summary$mean_ufboot), 
...
loci_boot73 <- dash_55_na_ng_summary %>%
  dplyr::filter(mean_ufboot >= 72.9 & No_variable_sites >= 100) %>%
  dplyr::pull(loci_names_198)
# Amino acids
loci_boot60 <- dash_55_aa_ng_summary %>%
  dplyr::filter(mean_ufboot >= 59.4 & No_variable_sites >= 100) %>%
  dplyr::pull(loci_names_198)
...
plot_figS2 <- function(df, yintercept) {
  df %>%
    ggplot2::ggplot(aes(x = No_variable_sites, y = mean_ufboot)) +
    ggplot2::geom_point() +
    ggplot2::geom_hline(yintercept = yintercept, linetype = "dashed", col = "red") +
    ggplot2::geom_vline(xintercept = 100, linetype = "dashed", col = "red") +
    ggplot2::scale_y_continuous(n.breaks = 10) +
    ggplot2::scale_x_continuous(n.breaks = 10) +
    ggplot2::labs(x = "No. of variable sites", y = "Mean UFboot") +
...
# Arrange panels
figS2 <- ggpubr::ggarrange(figS2a, figS2b, nrow = 1, ncol = 2)
# save figure
ggplot2::ggsave(filename = "figS2.pdf", plot = figS2, device = "pdf")
